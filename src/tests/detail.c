...
void print_insn(cs_insn *instructions, size_t count) {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    for (size_t j = 0; j < insn->size; j++) {
      printf("%d%s", insn->bytes[j], (j == insn->size - 1) ? "]\n" : ", ");
    }
    printf("  }%s\n", (i == count - 1) ? "\n]\n" : ",");
  }
}

void test_arch(arch *architecture) {
  csh handle;
  cs_insn *insn;
  size_t count;
  ...
  count = cs_disasm(handle, (uint8_t *)architecture->buffer,
                    sizeof(architecture->buffer) - 1, 0x1000, 0, &insn);
...
int main(void) {
  ...
  if (x86count > 0) {
    ...
    for (j = 0; j < x86count; j++) {
      ...
      printf("\n");
      printf("insn_offset_0: %d\n", sizeof(X86_REL_ADDR(x86insn[0])));
      printf("insn_offset_1: %d\n", sizeof(X86_REL_ADDR(x86insn[1])));
  ...
  /*
    ...
    if (armcount > 0) {
      ...
      printf("insn_offset: %ld\n", CS_INSN_OFFSET(arminsn, 13));
