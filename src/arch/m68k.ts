export interface cs_m68k_op {
  ...
  dimm?: number; // double imm
  simm?: number; // float imm
  reg?: M68K; // register value for REG operand
  reg_pair?: {
  ...
  mem?: {
    ...
    base_reg: M68K; // base register (or M68K.REG_INVALID if irrelevant)
    index_reg: M68K; // index register (or M68K.REG_INVALID if irrelevant)
    in_base_reg: M68K; // indirect base register (or M68K.REG_INVALID if irrelevant)
    in_disp: number; // indirect displacement
    out_disp: number; // other displacement
    disp: number; // displacement value
    scale: number; // scale for index register
    bitfield: number; // set to true if the two values below should be used
    width: number; // used for bf* instructions
    offset: number; // used for bf* instructions
    index_size: number; // 0 = w, 1 = l
  };
  br_disp?: {
    // data when operand is a branch displacement
    disp: number; // displacement value
    disp_size: number; // Size from m68k_op_br_disp_size type above
  };
  register_bits?: number; // register bits for movem etc. (always in d0-d7, a0-a7, fp0 - fp7 order)
  type: M68K;
  address_mode: M68K; // M68K addressing mode for this op
}
...
export enum M68K {
  ...
  OP_FP_SINGLE, // single precision Floating-Point operand
  OP_FP_DOUBLE, // double precision Floating-Point operand
  OP_REG_BITS, // Register bits move
  OP_REG_PAIR, // Register pair in the same op (upper 4 bits for first reg, lower for second)
  OP_BR_DISP, // Branch displacement
  ...
  GRP_ENDING = 8, // mark the end of the list of groups
}
...
export class cs_m68k {
  ...
  constructor(arch_info_ptr: number, Memory: any) {
    this.operands = [];
    this.op_count = Memory.read(arch_info_ptr + 232, 'ubyte');
    this.op_size = {
      type: Memory.read(arch_info_ptr + 224, 'i32'),
    };
    switch (this.op_size.type) {
      case M68K.SIZE_TYPE_CPU:
        this.op_size.cpu_size = Memory.read(arch_info_ptr + 228, 'i32');
        break;
      case M68K.SIZE_TYPE_FPU:
        this.op_size.fpu_size = Memory.read(arch_info_ptr + 228, 'i32');
        break;
    }
    for (let i = 0; i < this.op_count; i++) {
      const op: cs_m68k_op = {} as cs_m68k_op;
      const op_ptr: number = arch_info_ptr + i * 56;
